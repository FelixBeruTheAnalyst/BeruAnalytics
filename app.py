...
if page == "🏠 Home":
    ...
    with col6:
        ...
        <div class='feature-card'>
            ...
            PDF reports or filtered CSV files. 
    ...
    # Stats row
    st.markdown("<div class='section-header'>📈 Platform Capabilities</div>",
                unsafe_allow_html=True)
...
# ================================================
    ...
    with st.sidebar:
        ...
        for col in categorical_cols[:2]:
            ...
            if selected_cats:
                filtered_df = filtered_df[
                    filtered_df[col].isin(selected_cats)
                ]
    ...
    if numeric_cols and categorical_cols:
        ...
        with col1:
            ...
            x_col = st.selectbox("X Axis (Category)",
                                  categorical_cols, key="bar_x")
            y_col = st.selectbox("Y Axis (Value)",
        ...
        with col2:
            ...
            pie_col = st.selectbox("Category Column",
                                    categorical_cols, key="pie_col")
            pie_val = st.selectbox("Value Column",
                                    numeric_cols, key="pie_val")

            df_pie = filtered_df.groupby(pie_col)[pie_val].sum().reset_index()

            fig_pie = go.Figure(go.Pie(
                labels=df_pie[pie_col],
                values=df_pie[pie_val],
                hole=0.4,
    ...
    if len(numeric_cols) >= 1:
        ...
        if line_cols:
            fig_line = go.Figure()
            colors_line = ['#1A6B72', '#E8590C', '#534AB7',
                           '#2ecc71', '#e74c3c']
            for i, col in enumerate(line_cols):
                fig_line.add_trace(go.Scatter(
                    ...
                    mode='lines+markers',
                    name=col,
                    line=dict(color=colors_line[i % len(colors_line)],
                             width=2),
                    marker=dict(size=5),
...
# ================================================
    ...
    st.markdown("""
    Upload your data, describe what it contains, and BeruDataNarrate will generate
    a professional analysis report with charts, insights and recommendations —
    powered by AI.
    """)
    ...
    if st.session_state.df is not None:
        ...
        with col1:
            dataset_description = st.text_area(
                ...
                placeholder="Example: Monthly sales data for our Kenya retail stores showing revenue by region and product category for 2024...",
                height=120
            )
        with col2:
            user_question = st.text_area(
                "❓ What insights do you need? (optional)",
                placeholder="Example: Which regions are underperforming? What are the main revenue drivers?",
                height=120
            )
                ...
                if st.session_state.analysis and chart_paths:
                    with st.spinner("📝 Generating Word and PDF reports..."):
                        try:
                            ...
                            def add_word_section(title, content_text=None,
                                                  items=None):
                            ...
                            add_word_section("Executive Summary",
                                            analysis.get('executive_summary'))
                            add_word_section("Data Overview",
                                            analysis.get('data_overview'))
                            add_word_section("Key Findings",
                                            items=analysis.get('key_findings'))
                            add_word_section("Detailed Analysis",
                            ...
                            word_doc.add_page_break()
                            add_word_section("Recommendations",
                                            items=analysis.get('recommendations'))
                            add_word_section("Conclusion",
                            ...
                            fr = fp.add_run(
                                f"BeruAnalytics | Powered by AI | "
                                f"Built by Felix Beru Tsinzole"
                            )
                            fr.font.size = Pt(9)
                            ...
                            s_brand = ParagraphStyle('Brand', fontSize=26,
                                textColor=TEAL, alignment=TA_CENTER,
                                fontName='Helvetica-Bold', spaceAfter=8)
                            s_title = ParagraphStyle('Title', fontSize=18,
                                textColor=DARK_C, alignment=TA_CENTER,
                                fontName='Helvetica-Bold', spaceAfter=10)
                            s_sub = ParagraphStyle('Sub', fontSize=10,
                                textColor=GRAY_C, alignment=TA_CENTER,
                                fontName='Helvetica', spaceAfter=6)
                            s_sec = ParagraphStyle('Sec', fontSize=13,
                                textColor=TEAL, fontName='Helvetica-Bold',
                                spaceBefore=14, spaceAfter=6)
                            s_body = ParagraphStyle('Body', fontSize=11,
                                textColor=DARK_C, fontName='Helvetica',
                                spaceAfter=8, alignment=TA_JUSTIFY,
                                leading=16)
                            s_bullet = ParagraphStyle('Bul', fontSize=11,
                                textColor=DARK_C, fontName='Helvetica',
                                spaceAfter=5, leftIndent=20)
                            s_footer = ParagraphStyle('Foot', fontSize=9,
                                textColor=GRAY_C, alignment=TA_CENTER,
                            ...
                            pc.append(Spacer(1, 0.2*inch))
                            pc.append(HRFlowable(width="100%",
                                                  thickness=1, color=GRAY_C))
                            pc.append(PageBreak())

                            def add_pdf_section(title, text=None,
                                                 items=None):
                            ...
                            add_pdf_section("EXECUTIVE SUMMARY",
                                           analysis.get('executive_summary'))
                            add_pdf_section("DATA OVERVIEW",
                                           analysis.get('data_overview'))
                            add_pdf_section("KEY FINDINGS",
                                           items=analysis.get('key_findings'))
                            add_pdf_section("DETAILED ANALYSIS",
                                           analysis.get('detailed_analysis'))
                            pc.append(PageBreak())
                            ...
                            for cp in chart_paths:
                                if os.path.exists(cp):
                                    pc.append(Image(cp, width=5.5*inch,
                                                    height=3.5*inch))
                                    pc.append(Spacer(1, 0.2*inch))

                            pc.append(PageBreak())
                            add_pdf_section("RECOMMENDATIONS",
                                           items=analysis.get('recommendations'))
                            add_pdf_section("CONCLUSION",
                                           analysis.get('conclusion'))
                            pc.append(HRFlowable(width="100%",
                                                  thickness=1,
                                                  color=GRAY_C))
...
Answer the question clearly and specifically based on the data provided.
Be concise, professional and data-driven.
If you reference specific values, use the actual numbers from the data.
...
# ================================================
    ...
    with exp1:
        ...
        st.download_button(
            "📥 Download as CSV",
            csv_data,
            file_name="BeruAnalytics_Export.csv",
            mime="text/csv",
            use_container_width=True,
            type="primary"
        )
